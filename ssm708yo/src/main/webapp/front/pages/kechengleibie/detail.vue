...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','kechengleibie');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','kechengleibie');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('kechengleibie', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusskechengleibie/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('kechengleibie', this.detail);
				this.$utils.msg('审核成功');
